...
export default function ContactPageClient() {
  useEffect(() => {
    ...
    if (e.origin !== 'https://nexfield.pro') return;
    const d = e.data;
    const iframe = document.getElementById('nexfield-booking') as HTMLIFrameElement | null;
    if (!iframe) return;
    if (d && d.type === 'nexfield_resize') { iframe.style.height = d.height + 'px'; }
    if (d && d.type === 'nexfield_redirect' && d.url) { window.location.href = d.url; }
    };
            ...
            <div>
              <div className="bg-white rounded-3xl shadow-xl border border-gray-100 overflow-hidden">
                ...
                <div className="p-6 md:p-8">
                  <iframe
                    id="nexfield-booking"
                    src="https://nexfield.pro/crm/book?u=137&embed=1"
                    width="100%"
                    height={900}
                    style={{ border: 'none', borderRadius: '16px', maxWidth: '900px' }}
              ...
              <div className="mt-12">
                ...
                <div className="flex flex-wrap gap-2">
                  {LOCATIONS.slice(0, 8).map((location) => (
                    <span key={location.id} className="px-4 py-2 rounded-full bg-slate-100 text-gray-700 text-sm font-medium">{location.name}</span>
                  ))}
                  <span className="px-4 py-2 rounded-full bg-[#0b66b3]/10 text-[#0b66b3] text-sm font-medium">+ More Areas</span>
                </div>
              </div>
